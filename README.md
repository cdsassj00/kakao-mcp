...
사전에는 **관계망 지도**도 붙어 있습니다 — 임포트한 대화에서 누구와 얼마나
가까운지를 그래프와 강도(0~100)로 계산해 SVG 지도로 보여줍니다.
...
👤 "나랑 철수 얼마나 친해?" → relationship_strength(person: 철수)
```

- **노드** = 발화자, **엣지** = 인접 발화(창 3), **강도** = 빈도 × 최근성 감쇠(반감기 90일) × 상호성 보정
- LLM 추측이 아닌 결정론적 집계라 같은 입력에 항상 같은 결과
- `GET /map` 이 관계망 SVG를 렌더링 (본인 중심 원형 배치, 강도 = 선 굵기)
